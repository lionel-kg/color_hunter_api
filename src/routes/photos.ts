...
photosRouter.post('/:gameId', requireAuth, upload.array('photos', 9), async (req, res, next) => {
  try {
    const files = req.files as Express.Multer.File[] | undefined;
    if (!files || files.length === 0) throw new HttpError(400, 'Aucun fichier reçu');

    const game = await prisma.game.findUnique({ where: { id: req.params.gameId } });
    if (!game) throw new HttpError(404, 'Partie introuvable');
    if (game.status !== 'RUNNING') throw new HttpError(400, "La chasse n'est pas active");

    const participant = await prisma.gameParticipant.findUnique({
      where: { gameId_userId: { gameId: game.id, userId: req.user!.sub } },
    });
    if (!participant) throw new HttpError(403, 'Tu ne participes pas à cette partie');

    const quota = photoQuota(game.mode, game.teamSize);
    const existing = await prisma.photo.count({ where: { gameId: game.id, userId: req.user!.sub } });
    const slots = quota - existing;
    if (slots <= 0) throw new HttpError(400, `Quota atteint (${quota} photos max)`);
    if (files.length > slots) throw new HttpError(400, `Tu peux encore ajouter ${slots} photo${slots > 1 ? 's' : ''}`);

    const created = await Promise.all(files.map(async (file) => {
      // EXIF — vérifier que la photo est dans la fenêtre du défi
      let takenAt: Date | undefined;
      try {
        const exif = await exifr.parse(file.buffer, ['DateTimeOriginal', 'CreateDate']);
      ...
      const { url, key } = await uploadFile(file.buffer, `color-hunt/${game.id}`);
      return prisma.photo.create({
        data: { userId: req.user!.sub, gameId: game.id, cloudinaryUrl: url, cloudinaryPublicId: key, takenAt },
      });
    }));
...
photosRouter.post('/:gameId/grid', requireAuth, async (req, res, next) => {
  try {
    ...
    const photos = await prisma.photo.findMany({
      where: { id: { in: photoIds }, userId: req.user!.sub, gameId: req.params.gameId },
    });
    if (photos.length !== 9) throw new HttpError(400, 'Sélection invalide (9 photos requises)');
...
photosRouter.delete('/:photoId', requireAuth, async (req, res, next) => {
  try {
    ...
    if (photo.userId !== req.user!.sub) throw new HttpError(403, 'Pas autorisé');
