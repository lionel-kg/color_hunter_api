...
async function buildCompositeImage(photoUrls: string[]): Promise<Buffer> {
  ...
  return sharp({ create: { width: totalW, height: totalH, channels: 3, background: '#F5F0EB' } })
...
gridsRouter.post('/:gameId', requireAuth, async (req, res, next) => {
  try {
    const { photoIds, visibility } = z.object({
    ...
    if (game.status !== 'FINISHED') throw new HttpError(400, 'La partie n\'est pas encore terminée');
    ...
    if (existing) throw new HttpError(400, 'Tu as déjà créé une grille pour cette partie');

    const photos = await prisma.photo.findMany({
      where: { id: { in: photoIds }, userId: req.user!.sub, gameId: game.id },
    });
    if (photos.length !== 9) throw new HttpError(400, 'Sélection invalide — 9 photos requises et elles doivent t\'appartenir');
    ...
    const { url: imageUrl, key: imageKey } = await uploadFile(compositeBuffer, `color-hunt/grids/${game.id}`);
    ...
    const grid = await prisma.grid.create({
      data: {
        ...
        visibility,
        photos: {
          create: photoIds.map((photoId, idx) => ({ photoId, gridPosition: idx })),
        },
      },
      include: {
...
gridsRouter.get('/user/:userId', requireAuth, async (req, res, next) => {
  try {
    ...
    const visibilityFilter = meId === targetId
...
gridsRouter.patch('/:gridId/visibility', requireAuth, async (req, res, next) => {
  try {
    const { visibility } = z.object({
      visibility: z.enum(['PUBLIC', 'PRIVATE']),
    }).parse(req.body);

    const grid = await prisma.grid.findUnique({ where: { id: req.params.gridId } });
